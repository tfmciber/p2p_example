...
func ReceiveBenchhandler(stream network.Stream) {
	...
	appendToCSV("./bench.csv", []string{stream.Conn().ConnState().Transport, fmt.Sprintf("%d", numMessagesnum), fmt.Sprintf("%d", messageSizenum), fmt.Sprintf("%dgo ", elapsed.Milliseconds())})
